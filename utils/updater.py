...
class StrategyUpdater:
    ...
    def update_strategies(self, latest_hash=None, progress_callback=None):
        """Downloads updated .bat, .bin and list files from the repository."""
        try:
            # 1. Get the list of files in the repo root
            url_root = f"{self.api_base}/contents/"
            response_root = requests.get(url_root, timeout=10, verify=False)
            if response_root.status_code != 200:
                logger.error(f"Failed to get repo root contents: {response_root.status_code}")
                return False

            # 2. Get the list of files in the bin/ folder
            url_bin = f"{self.api_base}/contents/bin"
            response_bin = requests.get(url_bin, timeout=10, verify=False)
            bin_files_list = []
            if response_bin.status_code == 200:
                bin_files_list = response_bin.json()
            else:
                logger.warning(f"Failed to get repo bin/ contents: {response_bin.status_code}")

            # 3. Get the list of files in the utils/ folder
            url_utils = f"{self.api_base}/contents/utils"
            response_utils = requests.get(url_utils, timeout=10, verify=False)
            utils_files_list = []
            if response_utils.status_code == 200:
                utils_files_list = response_utils.json()
            else:
                logger.warning(f"Failed to get repo utils/ contents: {response_utils.status_code}")

            # 4. Get the list of files in the .service/ folder (New in v1.9.1)
            url_service = f"{self.api_base}/contents/.service"
            response_service = requests.get(url_service, timeout=10, verify=False)
            service_files_list = []
            if response_service.status_code == 200:
                service_files_list = response_service.json()
            else:
                logger.warning(f"Failed to get repo .service/ contents: {response_service.status_code}")

            files_root = response_root.json()
            
            # Filter relevant files from root
            # - .bat files (strategies)
            # - .txt files (lists)
            strategies_to_update = [f for f in files_root if (f.get("name", "").startswith("general") and f.get("name", "").endswith(".bat")) or f.get("name", "") == "service.bat"]
            lists_to_update = [f for f in files_root if f.get("name", "").endswith(".txt") and ("exclude" in f.get("name", "") or "list-" in f.get("name", ""))]
            
            # Filter .bin, .exe, .sys, .dll files from bin/ folder
            bins_to_update = [f for f in bin_files_list if f.get("name", "").endswith((".bin", ".exe", ".sys", ".dll"))]

            # Filter relevant files from utils/ folder
            utils_to_update = [f for f in utils_files_list if f.get("name", "") in ["targets.txt", "GameFilter"]]

            # Filter relevant files from .service/ folder
            service_folder_files = [f for f in service_files_list if f.get("type") == "file"]

            # Combine all files for the update process
            all_files = []
            for f in strategies_to_update: all_files.append((f, self.zapret_path))
            for f in lists_to_update: all_files.append((f, os.path.join(self.zapret_path, "lists")))
            for f in bins_to_update: all_files.append((f, os.path.join(self.zapret_path, "bin")))
            for f in utils_to_update: all_files.append((f, os.path.join(self.zapret_path, "utils")))
            for f in service_folder_files: all_files.append((f, os.path.join(self.zapret_path, ".service")))
            ...
            for i, (file_info, target_dir) in enumerate(all_files):
                ...
                path_in_repo = file_info.get("path", name)
                
                if progress_callback:
                    progress_callback(i, total_files, name)
                
                download_url = f"{self.raw_base}/{path_in_repo}"
                ...
                # Backup existing file (only for .bat files)
                if name.endswith(".bat") and os.path.exists(target_path):
                ...
                try:
                    ...
                    if file_response.status_code == 200:
                        ...
                        updated_count += 1
                        logger.info(f"Updated: {name} in {target_dir}")
                    else:
                        logger.error(f"Failed to download {name}: {file_response.status_code}")
                except Exception as download_err:
                    logger.error(f"Error downloading {name}: {download_err}")
