...
class AppController:
    ...
    def install_runtime_latest(self, progress_callback=None):
        # If already present, nothing to do
        if os.path.exists(os.path.join(self.bin_path, "winws.exe")):
            return True
        has_update, latest_ref = updater.check_for_updates()
        ref = latest_ref or "main"
        return updater.download_runtime(ref, progress_callback)
