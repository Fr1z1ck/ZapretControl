...
class SettingsDialog(QDialog):
    ...
    def init_ui(self):
        ...
        last_ver = config.get("last_update_version")
        last_hash = config.get("last_update_hash")
        last_str = last_ver or (last_hash[:7] if last_hash else "неизвестно")
        self.update_status_label = QLabel("Стратегии: " + last_str)
    ...
    def on_update_finished(self, success, error):
        ...
        if success:
            ...
            last_hash = config.get("last_update_hash")
            last_str = last_ver or (last_hash[:7] if last_hash else "неизвестно")
            self.update_status_label.setText("Последнее обновление: " + last_str)
            self.parent().update_strategies_ui()
...
class RuntimeThread(QThread):
    ...
    def run(self):
        try:
            success = self.controller.install_runtime_latest(self.on_progress)
            self.finished.emit(success, "")
        except Exception as e:
            self.finished.emit(False, str(e))
    def on_progress(self, current, total, filename):
...
class HealthCheckThread(QThread):
    ...
    def run(self):
        try:
            ...
            # Emit a progress line to make the error visible in the UI log
            try:
                self.progress.emit(0, 0, f"Ошибка проверки: {str(e)}")
            except Exception:
                pass
        finally:
...
class MainWindow(QMainWindow):
    ...
    def _runtime_exists(self):
        exe_path = get_resource_path(os.path.join('Zapret', 'bin', 'winws.exe'))
        return os.path.exists(exe_path)

    def _prompt_runtime_missing(self):
        msg = QMessageBox(self)
        msg.setIcon(QMessageBox.Warning)
        msg.setWindowTitle("Не найден winws.exe")
        msg.setText("Требуется Zapret runtime (winws.exe) в папке Zapret/bin.")
        msg.setInformativeText("Откройте страницу релизов и распакуйте архив в папку Zapret.")
        msg.setStandardButtons(QMessageBox.Ok | QMessageBox.Open)
        ret = msg.exec()
        if ret == QMessageBox.Open:
            QDesktopServices.openUrl(QUrl("https://github.com/Flowseal/zapret-discord-youtube/releases/latest"))
        self.log("winws.exe не найден. Откройте релизы Flowseal и распакуйте архив в Zapret/bin.")
        # Попытка автоматической загрузки runtime
        try:
            self.log("Пробую автоматически скачать Zapret runtime...")
            self.runtime_thread = RuntimeThread(self.controller)
            self.runtime_thread.progress.connect(self.on_update_progress)
            self.runtime_thread.finished.connect(self.on_runtime_finished)
            self.runtime_thread.start()
        except Exception as e:
            self.log(f"Автозагрузка runtime не удалась: {str(e)}")

    def on_toggle_clicked(self):
        self.toggle_btn.setEnabled(False)
        if not self._runtime_exists():
            self._prompt_runtime_missing()
    ...
    def on_update_progress(self, current, total, filename):
        """Handle progress updates for runtime download."""
        if total > 0:
            percent = int((current / total) * 100)
            # Log only every 25% to avoid spamming the log view
            if percent % 25 == 0 or current == total:
                self.log(f"Загрузка {filename}: {percent}%")
        else:
            self.log(f"Загрузка: {filename}")

    def on_runtime_finished(self, success, error):
        """Handle completion of runtime download."""
        if success and self._runtime_exists():
            self.log("Zapret runtime успешно установлен.")
            # Re-enable buttons if they were waiting for runtime
            self.health_btn.setEnabled(True)
            self.check_one_btn.setEnabled(True)
            self.toggle_btn.setEnabled(True)
        else:
            self.log(f"Не удалось установить runtime: {error or 'ошибка загрузки'}")
            self.log("Пожалуйста, установите Zapret runtime вручную.")
    ...
    def on_health_check_clicked(self, check_all=True):
        ...
        # Determine strategy index robustly
        if check_all:
            strategy_index = None
        else:
            idx = self.strategy_combo.currentIndex()
            if idx is None or idx < 0:
                idx = 0 if len(self.controller.strategies) > 0 else None
            strategy_index = idx
